...
EXTRACT = Path(r"C:\HypercubeEtalon\RamanModels\extracted")
...
def load_row(path: Path) -> np.ndarray:
    if not path.is_file():
        raise SystemExit(f"missing {path} (run etalon_raman_extract first)")
...
def main() -> None:
    ...
    if not manifest.is_file():
        raise SystemExit(f"missing {manifest} (run etalon_raman_extract first)")
